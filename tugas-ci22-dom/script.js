...
function validatePassword(name, password) {
    ...
    if(password.length >= minLength && password.match(upperCase) && password.match(lowerCase) && password.match(number)) {
        alert(`Login berhasil, Selamat datang ${name}!`)
        clearInput();
    } else {
        alert("Password harus terdiri dari minimal 8 karakter dengan kombinasi huruf besar, huruf kecil, dan angka.")
    }
}
